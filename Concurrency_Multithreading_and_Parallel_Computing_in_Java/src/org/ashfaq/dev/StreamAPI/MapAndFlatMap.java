...
public class MapAndFlatMap {
	...
	public static void main(String[] args) {
		...
		List<Integer> list = bookList.stream().map(x -> x.getTitle().length()).toList();
//		System.out.println(list);

		// flatmap() -mapping each array not within a stream but with the content of
		// that stream
		// eg [ [1,2,3],[5,13] ] --> [1,2,3,5,13]

		String[] arr = { "Shell", "hello" };

		Arrays.stream(arr).map(x -> x.split("")).flatMap(x -> Arrays.stream(x)) // to flatten the arrays
